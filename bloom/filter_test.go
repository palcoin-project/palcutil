...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := palcutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(palcutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := palcutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := palcutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
