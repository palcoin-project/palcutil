...
func (cs *CoinSet) TotalValue() (value palcutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue palcutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue palcutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount palcutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue palcutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue palcutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue palcutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue palcutil.Amount, coins []Coin) (Coins, error) {
...
// palcutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() palcutil.Amount {
	return palcutil.Amount(c.txOut().Value)
}
