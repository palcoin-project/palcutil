...
import (
	...
	"github.com/palcoin-project/palcutil/base58"
)
...
func (k *ExtendedKey) Derive(i uint32) (*ExtendedKey, error) {
	...
	parentFP := palcutil.Hash160(k.pubKeyBytes())[:4]
...
func (k *ExtendedKey) CloneWithVersion(version []byte) (*ExtendedKey, error) {
	if len(version) != 4 {
		...
		//  ErrInvalidHDKeyID (introduced in palcoin-project/palcd#1617). Update the
...
func (k *ExtendedKey) Address(net *chaincfg.Params) (*palcutil.AddressPubKeyHash, error) {
	pkHash := palcutil.Hash160(k.pubKeyBytes())
	return palcutil.NewAddressPubKeyHash(pkHash, net)
}
